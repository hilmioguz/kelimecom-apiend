...
const { Previewmadde } = require('../models');
 ...
 * @returns {Promise<Previewmadde>}
 */
const createMadde = async (maddeBody) => {
  if (await Previewmadde.isMaddeAlrearyInDB(maddeBody.madde)) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Madde zaten tanımlı');
  }
  const madde = await Previewmadde.create(maddeBody);
  return madde;
};

const createSubMadde = async (maddeId, maddeBody) => {
  const madde = await Previewmadde.updateOne(
...
const queryMaddeler = async (filter, options) => {
  ...
  if (options.type && options.type === 'searchText') {
    maddeler = await Previewmadde.find(filter, { score: { $meta: 'textScore' } }).sort({ score: { $meta: 'textScore' } });
  } else {
    maddeler = await Previewmadde.paginate(filter, options);
  }
  return maddeler;
};
...
const rawQueryMaddeler = async (madde) => {
  ...
  // eslint-disable-next-line no-unused-vars
  const maddeler = await Previewmadde.aggregate(agg, (cmdErr, result) => {
...
const getMaddeById = async (id) => {
  return Previewmadde.findById(id);
};
...
const getMaddeByName = async (madde) => {
  return Previewmadde.findOne({ madde });
};
...
const updateMaddeById = async (maddeId, updateBody) => {
  ...
  if (updateBody.madde && (await Previewmadde.isMaddeAlrearyInDB(updateBody.madde, maddeId))) {
...
const deleteMaddeById = async (maddeId) => {
  const madde = await Previewmadde.deleteOne({ _id: maddeId });
  if (!madde) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Madde bulunamadı');
  }
  return madde;
...
const userMaddeFavorites = async (id, anlamId, userId, method) => {
  let sonuc = null;
  if (method === 'insert') {
    sonuc = await Previewmadde.updateOne(
